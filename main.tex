...
\caption{The Cross correlation profiles obtained by simulating the Fermi/GBM light curves of the long GRB 130502327 and of the short GRB 120323507 using the templates shown in \autoref{fig:template_short_long} for each burst. In each panel we also report an inset showing a zoom of the cross correlation profiles around the peak, and the best fit function resulting by fitting the cross correlation profile in this region with a Gaussian function (red solid line in the left panel) and with a double-exponential function (red solid line in the right panel). For the reported cross correlation profiles we used a bin time of 100 \us. } 
